...
% --- Executes on button press in ChallengeButton.
function ChallengeButton_Callback(hObject, eventdata, handles)
% hObject    handle to ChallengeButton (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

%The distance is asked from the textfield
...
while(1)
    ...
    if(strcmp('return', start)) %if the enter button is pushed, challenge 1 will run
        drive2([interx1, intery1; interx2, intery2; interx3, intery3], beginangle, beginx, beginy);
        break
    else
